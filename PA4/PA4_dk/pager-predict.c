...
static void pred_pageit(Pentry q[MAXPROCESSES], uint32_t tick){
	...
	// done with process if not active
    for(proc = 0; proc < MAXPROCESSES; proc++){
        if(!q[proc].active){
            continue;
        }
		// build free and not free process table
		...
		// create page and timestamp for page
        page = q[proc].pc/PAGESIZE;
        timestamps[proc][page] = tick;
		// if page is already in memory then done since cant do anything with it anymore
        ph_r.page = page;
        if(q[proc].pages[page]){
            ph_r.fault = 0;
		...
		// if process has more pages allocated than needed, then take them away and free them
		...
		// if no more free pages then make sure there is at least one page to evict and to not evict more than once per 100ms to prevent starvation and aggressive eviction
    ...
    assert(free_phys >= 0);
	// free as many pages as there are starving processes that were swapped in
    if(amount_unsat > 0) {     
        
        if(pages_freed >= amount_unsat)
            return;
		// order fault table by proccess
        ...
        // free as many pages as there are unsatisfied (thrashed) processes
        int k;
...
// more of a forecasting algorithm than a predictive one. Uses lru history to forecast which processes will need to be run soon and makes sure that there are enough free pages to support thrashed or to be thrashed processes
